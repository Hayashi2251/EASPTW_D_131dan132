...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('product.update',$product->id)}}">
        ...
        <div class="form-group">
          <label for="is_featured">Diunggulkan</label><br>
          <input type="checkbox" name='is_featured' id='is_featured' value='{{$product->is_featured}}' {{(($product->is_featured) ? 'checked' : '')}}> YA                        
        </div>
              {{-- {{$categories}} --}}
        ...
        <div class="form-group">
          <label for="price" class="col-form-label">Harga (Rupiah) <span class="text-danger">*</span></label>
        ...
        <div class="form-group">
          <label for="discount" class="col-form-label">Masukkan Harga</label>
        ...
        <div class="form-group">
          ...
          <select name="size[]" class="form-control selectpicker"  multiple data-live-search="true">
              ...
              @foreach($items as $item)              
                ...
                // dd($data);
                @endphp
              <option value="S"  @if( in_array( "S",$data ) ) selected @endif>Kecil</option>
              <option value="M"  @if( in_array( "M",$data ) ) selected @endif>Medium</option>
              <option value="L"  @if( in_array( "L",$data ) ) selected @endif>Besar</option>
              <option value="XL"  @if( in_array( "XL",$data ) ) selected @endif>Extra Besar</option>
              @endforeach
          </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label for="stock">Quantitas <span class="text-danger">*</span></label>
          <input id="quantity" type="number" name="stock" min="0" placeholder="Masukkan Quantitas"  value="{{$product->stock}}" class="form-control">
          @error('stock')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
                    ...
                    success:function(response){
                        ...
                        if(response.status){
                            ...
                            if(response.data){
                                ...
                                console.log('tidak ada data tanggapan');
                            }
                        }
                        else{
