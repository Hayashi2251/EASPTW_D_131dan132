...
@section('main-content')
 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary float-left">List Kupon</h6>
      <a href="{{route('coupon.create')}}" class="btn btn-primary btn-sm float-right" data-toggle="tooltip" data-placement="bottom" title="Add User"><i class="fas fa-plus"></i> Tambahkan Kupon</a>
    </div>
    <div class="card-body">
      <div class="table-responsive">
                        ...
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title" id="#delModal{{$user->id}}Label">Delete Pengguna</h5>
                            ...
                            <div class="modal-body">
                              <form method="post" action="{{ route('banners.destroy',$user->id) }}">
                                @csrf 
                                @method('delete')
                                <button type="submit" class="btn btn-danger" style="margin:auto; text-align:center">Delete Permanen Pengguna</button>
                              </form>
                            </div>
                          </div>
                        </div>
        ...
        @else
          <h6 class="text-center">Tidak ada Kupon ditemukan! mohon buat kupon</h6>
        @endif
      </div>
    </div>
</div>
          ...
          $('.dltBtn').click(function(e){
            var form=$(this).closest('form');
              ...
              swal({
                    ...
                    text: "setelah dihapus, data tidak dapat di recover!",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
                .then((willDelete) => {
                    if (willDelete) {
                       form.submit();
                        ...
                        swal("Ydata anda aman data is safe!");
                    }
                });
          })
      })
